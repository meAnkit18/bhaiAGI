...
# パッケージマネージャー
npm i -g bhaiagi-ai@latest        # bun/pnpm/yarn でもOK
scoop install bhaiagi             # Windows
choco install bhaiagi             # Windows
brew install anomalyco/tap/bhaiagi # macOS と Linux（推奨。常に最新）
brew install bhaiagi              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S bhaiagi            # Arch Linux (Stable)
paru -S bhaiagi-bin               # Arch Linux (Latest from AUR)
mise use -g bhaiagi               # どのOSでも
nix run nixpkgs#bhaiagi           # または github:anomalyco/bhaiagi で最新 dev ブランチ
```
...
bhaiagi はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/bhaiagi/releases) から直接ダウンロードするか、[bhaiagi.ai/download](https://bhaiagi.ai/download) を利用してください。
...
4. `$HOME/.bhaiagi/bin` - デフォルトのフォールバック
...
bhaiagi には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://bhaiagi.ai/docs/agents) の詳細はこちら。

### ドキュメント

bhaiagi の設定については [**ドキュメント**](https://bhaiagi.ai/docs) を参照してください。

### コントリビュート

bhaiagi に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### bhaiagi の上に構築する

bhaiagi に関連するプロジェクトで、名前に "bhaiagi"（例: "bhaiagi-dashboard" や "bhaiagi-mobile"）を含める場合は、そのプロジェクトが bhaiagi チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[bhaiagi Zen](https://bhaiagi.ai/zen) で提供しているモデルを推奨しますが、bhaiagi は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
- TUI にフォーカス。bhaiagi は neovim ユーザーと [terminal.shop](https://terminal.shop) の制作者によって作られており、ターミナルで可能なことの限界を押し広げます。
- クライアント/サーバー構成。例えば bhaiagi をあなたのPCで動かし、モバイルアプリからリモート操作できます。TUI フロントエンドは複数あるクライアントの1つにすぎません。

---

**コミュニティに参加** [Discord](https://discord.gg/bhaiagi) | [X.com](https://x.com/bhaiagi)
