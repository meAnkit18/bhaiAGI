...
brew install anomalyco/tap/bhaiagi # macOS 和 Linux（推荐，始终保持最新）
brew install bhaiagi              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S bhaiagi            # Arch Linux (Stable)
paru -S bhaiagi-bin               # Arch Linux (Latest from AUR)
mise use -g bhaiagi               # 任意系统
nix run nixpkgs#bhaiagi           # 或用 github:anomalyco/bhaiagi 获取最新 dev 分支
```
...
bhaiagi 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/bhaiagi/releases) 或 [bhaiagi.ai/download](https://bhaiagi.ai/download) 下载。
...
bhaiagi 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://bhaiagi.ai/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://bhaiagi.ai/docs)。
...
如果你在项目名中使用了 “bhaiagi”（如 “bhaiagi-dashboard” 或 “bhaiagi-mobile”），请在 README 里注明该项目不是 bhaiagi 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [bhaiagi Zen](https://bhaiagi.ai/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。bhaiagi 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
- 客户端/服务器架构。可在本机运行，同时用移动设备远程驱动。TUI 只是众多潜在客户端之一。

---

**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=738j8655-cd59-4633-a30a-1124e0096789&qr_code=true) | [X.com](https://x.com/bhaiagi)
