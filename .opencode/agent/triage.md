...
**Only** add if it's specifically about the Desktop application or `bhaiagi web` view. **Do not** add for terminal, TUI, or general bhaiagi issues.
...
**Only** add if the issue mentions "zen" or "bhaiagi zen" or "bhaiagi black".

If the issue doesn't have "zen" or "bhaiagi black" in it then don't add zen label

#### core

Use for core server issues in `packages/bhaiagi/`, excluding `packages/bhaiagi/src/cli/cmd/tui/`.
...
TUI (`packages/bhaiagi/src/cli/cmd/tui/...`):
...
Core (`packages/bhaiagi/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for bhaiagi server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
