...
export const UpgradeCommand = {
  ...
  describe: "upgrade bhaiagi to the latest or a specific version",
  builder: (yargs: Argv) => {
    return yargs
      .positional("target", {
  ...
  handler: async (args: { target?: string; method?: string }) => {
    ...
    if (method === "unknown") {
      prompts.log.error(`bhaiagi is installed to ${process.execPath} and may be managed by a package manager`)
    ...
    if (InstallationVersion === target) {
      prompts.log.warn(`bhaiagi upgrade skipped: ${target} is already installed`)
      prompts.outro("Done")
      return
    }
