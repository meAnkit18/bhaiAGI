...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall bhaiagi and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall bhaiagi")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g bhaiagi",
      pnpm: "pnpm uninstall -g bhaiagi",
      bun: "bun remove -g bhaiagi",
      yarn: "yarn global remove bhaiagi",
      brew: "brew uninstall bhaiagi",
      choco: "choco uninstall bhaiagi",
      scoop: "scoop uninstall bhaiagi",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using bhaiagi!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# bhaiagi") || content.includes(".bhaiagi/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".bhaiagi/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".bhaiagi/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".bhaiagi"))
    ) {
      continue
    }
