...
Replace runtime Bun shell template-tag usage in `packages/bhaiagi/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/bhaiagi/src` is removed except:
