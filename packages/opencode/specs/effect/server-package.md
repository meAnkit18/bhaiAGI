...
Practical reference for extracting a future `packages/server` from the current `packages/bhaiagi` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all bhaiagi services, Effect-first source of truth
- `packages/server` - bhaiagi server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full bhaiagi server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/bhaiagi`.
...
The intended direction here is the same, but the current `bhaiagi` package split is earlier in the migration.
...
- the current server host still lives in `packages/bhaiagi`
...
- if `packages/server` imports services from `packages/bhaiagi` while `packages/bhaiagi` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/bhaiagi` as the temporary Hono host
...
- `packages/server` must not import from `packages/bhaiagi`

Allowed in phase 1:

- `packages/bhaiagi` imports `packages/server`
...
- `packages/bhaiagi` shrinks or disappears as package responsibilities are fully split
...
- `packages/bhaiagi` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/bhaiagi`
...
Keep in `packages/bhaiagi` for now:
...
- host mounting from `packages/bhaiagi`
...
- no imports from `packages/bhaiagi` inside `packages/server`
...
- `packages/server` must still not import from `packages/bhaiagi`
...
### PR 4. Mount `packages/server` question from `packages/bhaiagi`

Scope:

- replace local experimental question route wiring in `packages/bhaiagi`
...
- mount it in parallel from `packages/bhaiagi`
...
- move adapter selection and server startup out of `packages/bhaiagi`

Rules:

- do not start this while `packages/server` still depends on `packages/bhaiagi`
...
- host logic no longer lives in `packages/bhaiagi`
...
3. the route is mounted from the current host in `packages/bhaiagi`
...
- `packages/server` imports services or runtime from `packages/bhaiagi`
- `packages/bhaiagi` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/bhaiagi` imports
...
- [ ] mount `question` from `packages/bhaiagi`
...
2. keep `packages/bhaiagi` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/bhaiagi`, stop and restructure the boundary first.
